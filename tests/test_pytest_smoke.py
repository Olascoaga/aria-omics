...
def test_qc_cache_requires_matching_parameters():
    ...
    assert _cache_matches(
        {"cache_version": 6, "cache_params": expected},
        expected,
    )
    assert "user_question" not in expected
    stale = {**expected, "min_genes": 500}
    assert not _cache_matches(
        {"cache_version": 6, "cache_params": stale},
        expected,
    )
    # Older cache schemas must not be reused after QC-threshold policy changes
    # (N-QC1 moved the count/gene MAD bounds to log space → bump 4→5; T6 added
    # the MAD=0 minimum-width fallback → bump 5→6).
    assert not _cache_matches(
...
def test_narrative_html_escapes_methods_and_uses_package_version(tmp_path):
    ...
    assert "Commit: <code>" in html
    assert "Dirty: <code>" in html
    assert "Workflow: <code>" in html
...
def test_version_stamps_are_derived_from_single_source():
    ...
    metadata = collect_version_metadata()
    assert metadata["aria_version"] == __version__
    assert metadata["version"] == __version__
    assert metadata["version_source"] == "aria.version.__version__"
    assert metadata["git_commit"] == metadata["git_sha"]
    assert re.fullmatch(r"[0-9a-f]{64}", metadata["workflow_hash"])

    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    assert f"![Version]({version_badge_url()})" in readme

    release_notes = ROOT / "docs" / f"release_notes_v{__version__}.md"
    assert release_notes.exists()
    assert f"v{__version__}" in release_notes.read_text(encoding="utf-8").splitlines()[0]
...
def test_provenance_block_contains_required_fields():
    ...
    for key in {
        ...
        "platform", "conda_env", "timestamp_utc", "version_source",
        "git_commit", "git_tree_sha", "git_describe", "workflow_hash",
...
def test_provenance_section_renders_llm_usage():
    ...
    html = agent._build_provenance_section(
        provenance={
            ...
            "git_describe": "v-test",
            "workflow_hash": "f" * 64,
            "workflow_hash_algorithm": "sha256(test)",
