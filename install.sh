...
echo -e "${BLD}  Installer v0.1.0 -- Ubuntu / WSL2${RST}"
echo ""
...
if [ -n "$ANTHROPIC_KEY" ] && [ -n "$GEMINI_KEY" ]; then
    ...
    warn "No API keys configured. Add them later in ~/.aria/config.yaml"
    PRIMARY_PROVIDER=0
fi
...
if [ -n "$GEMINI_KEY" ]; then
    ...
    grep -q "GEMINI_API_KEY" "$HOME/.bashrc" || \
        echo "export GEMINI_API_KEY=${GEMINI_KEY}"  >> "$HOME/.bashrc"
    grep -q "GOOGLE_API_KEY" "$HOME/.bashrc" || \
        echo "export GOOGLE_API_KEY=${GEMINI_KEY}"  >> "$HOME/.bashrc"
...
if python "$ARIA_DIR/tests/test_integration.py"; then
    ...
    warn "ARIA can still run but some components may have issues"
fi
