...
| `raw_ingestion_agent.py` FASTQ/kb execution + checkpoint / `rna_kb_count.py` / `raw_ingestion.execute_kb_count` | `EnvironmentManager`, `SCRIPT_CONTRACTS`, TUI/headless checkpoint resolution, `report_sections._build_raw_ingestion_section`, scRNA modality handoff | ADR-029: scRNA FASTQ raw closure gates deeper ATAC work. FASTQ quantification must run through `EnvironmentManager.run_in_stack(stack="rna", script_path="aria/scripts/rna_kb_count.py")`, not direct in-process `subprocess.run` from the agent. `rna_kb_count.py` is beta and wraps the deterministic helper; incomplete script inputs map to structured `KbInputBlocked`, successful `fastq_kb_count` outputs become canonical `scRNA_ingested_h5ad` inputs. When FASTQs are detected and no executable `raw_ingestion_kb` params are supplied, `RawIngestionAgent` publishes blocking checkpoint `raw_ingestion.fastq_kb`: default/headless skips quantification, `Other` may paste explicit JSON, and cancellation stops raw ingestion. Do not silently infer chemistry/reference/indexes or let a raw FASTQ-only scRNA run fall through as if a canonical `.h5ad` existed. |
