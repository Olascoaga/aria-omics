...
| `data_audit_agent.py` SIGNATURES / `_scan_directory` bounded scan | modality classification, CP1, `.h5mu`/chromatin routing | `.h5mu` (paired RNA+ATAC) is scanned and classified as `scATAC` (C8); changing the signature order or extension set can make the v4.6 entry input undetectable again. P2-7 replaces unbounded `rglob("*")` with `DataAuditScanLimits`: configurable max files/entries/depth/time/file-size, symlink opt-in, visited-directory protection, and CP1 warnings when the scan is truncated or skips paths. Keep defaults permissive enough for valid BAM/H5 inputs, and do not silently follow symlinks. |
