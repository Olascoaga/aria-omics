...
| `report_builder._govern_executive_summary` / `NarrativeAgent._fallback_executive_summary` / `validators.find_causal_language` | Executive Summary HTML for all modalities, especially modalities without deterministic executive-summary builders (chromatin/scATAC) | C1a post-v4.6 audit remediation: free-text LLM executive summaries are guarded immediately before HTML render. The guard redacts collected external named entities, scans for unlicensed causal language, and rejects numeric claims whose normalized values are absent from the concrete agent-results summary (`_summarize_agent_results_for_llm`). On violation, the report shows the existing deterministic fallback plus a visible "Executive summary governance" warning. A summary that is already the fallback is not reflagged for its own counters. Guard: `tests/test_executive_summary_governance.py`. C1b will later model the summary as a first-class `NarrativeBlock`; until then this is the render-level mitigation. |
