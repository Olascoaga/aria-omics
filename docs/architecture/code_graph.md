...
| `raw_ingestion_agent.py` FASTQ/kb execution / `rna_kb_count.py` / `raw_ingestion.execute_kb_count` | `EnvironmentManager`, `SCRIPT_CONTRACTS`, `report_sections._build_raw_ingestion_section`, scRNA modality handoff | ADR-029: scRNA FASTQ raw closure gates deeper ATAC work. FASTQ quantification must run through `EnvironmentManager.run_in_stack(stack="rna", script_path="aria/scripts/rna_kb_count.py")`, not direct in-process `subprocess.run` from the agent. `rna_kb_count.py` is beta and wraps the deterministic helper; incomplete inputs map to structured `KbInputBlocked`, successful `fastq_kb_count` outputs become canonical `scRNA_ingested_h5ad` inputs. Do not silently infer chemistry/reference/indexes or let a raw FASTQ-only scRNA run fall through as if a canonical `.h5ad` existed. |
