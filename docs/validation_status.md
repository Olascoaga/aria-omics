...
| Bulk ATAC peak-count matrix | Scaffold opened | V47 comparison requests now build a peak-by-sample TSV from called/consensus peaks with `bedtools coverage -counts`, register it in the chromatin narrator and run ledger, and still skip bulk ATAC differential accessibility with `bulk_atac_da_not_validated` until the replicate-aware DA lane is separately validated |
