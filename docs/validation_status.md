...
| Air-gapped mode | Partial | `ARIA_AIR_GAPPED=1` gates egress across the LLM, pathway ORA, and GEO/SRA connectors (not just the LLM) and redacts failed-run input/error archives. Known gap (preprint audit A1/A3): the flag is resolved at provider construction and the setup path can still shell out, so a per-run egress context checked at every call is being hardened before the "governs all egress" guarantee is claimed |
...
| GEO/SRA connector | Beta | GSE183948 path validated; multi-organism (spike-in) organism inference added; **recognizes + routes all four ARIA modalities (scRNA/bulk_RNA/scATAC/bulk_ATAC) — real-validated on GSE96769 (full fetch) + GSE162690/GSE129785/GSE47753 (listing classification).** Scope caveat (preprint audit E6): this is classification/routing of already-processed supplementary files, NOT general reproducible retrieval of all four modalities — raw FASTQ from SRA is left `fastq_pending` (external tooling), and robust archive extraction/checksums are not yet in place; public metadata remains heterogeneous |
